...
//go:generate go run C:\Users\dur4n\go\pkg\mod\golang.org\x\sys@v0.7.0\windows\mkwinsyscall\ -output zsyscalls_windows.go syscalls_windows.go

//sys OpenProcess(da uint32, inheritHandle bool, pid uint32) (handle windows.Handle, err error) = advapi32.OpenProcess
//sys OpenProcessToken(h windows.Handle, access uint32, token *windows.Token) (err error) = advapi32.OpenProcess
//sys GetTokenInformation(t windows.Token, infoClass uint32, info *byte, infoLen uint32, returnedLen *uint32) (err error) = advapi32.OpenProcess
