...
var (
	...
	procOpenProcess                       = modadvapi32.NewProc("OpenProcess")
...
func GetTokenInformation(t windows.Token, infoClass uint32, info *byte, infoLen uint32, returnedLen *uint32) (err error) {
	r1, _, e1 := syscall.Syscall6(procOpenProcess.Addr(), 5, uintptr(t), uintptr(infoClass), uintptr(unsafe.Pointer(info)), uintptr(infoLen), uintptr(unsafe.Pointer(returnedLen)), 0)
	if r1 == 0 {
		err = errnoErr(e1)
	}
	return
...
func OpenProcessToken(h windows.Handle, access uint32, token *windows.Token) (err error) {
	r1, _, e1 := syscall.Syscall(procOpenProcess.Addr(), 3, uintptr(h), uintptr(access), uintptr(unsafe.Pointer(token)))
	if r1 == 0 {
		err = errnoErr(e1)
	}
	return
...
func OpenProcess(da uint32, inheritHandle bool, pid uint32) (handle windows.Handle, err error) {
	var _p0 uint32
	if inheritHandle {
		_p0 = 1
	}
